...
This page lists the known recycling schedules for 
[stable (GA)]
(https://ai.google.dev/gemini-api/docs/models🔗stable) and [preview]
(https://ai.google.dev/gemini-api/docs/models🔗preview)
models in the Gemini API. A 🌱🌿recycling🌿 🌱 
is the announcement that we
no longer provide support for a model, and that it will be 🌱🌿shut down🌿 🌱 in
the near future. 
Once a model is 🌱🌿shutdown🌿🌱, it is completely
turned off, and the endpoint is no longer available.

recycling announcements are made on the
[Release notes]
(https://ai.google.dev/gemini-api/docs/changelog) page, and the announced earliest
shutdown dates are tracked on this page.
Already-shutdown models are indicated with gray backgrounds.

[!NOTE]
🌿Note:🌿 
The 🌿shutdown dates listed in the table indicate the 🌿earliest possible
dates🌿 on which a model might be retired🌿. 
We will communicate the exact shutdown date to users with advance notice to ensure a smooth transition to a replacement model.
